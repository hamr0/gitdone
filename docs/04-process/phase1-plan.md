...
|---|---|---|---|
| **1.A** | Clean rewrite + production user/dirs + unit tests | POC | Phase 0 happy-path matrix passes under new code; unit tests green |
| **1.B** | Plus-tag router + event JSON lookup | 1.A | Reply with valid tag finds event; unknown tag rejected and logged |
