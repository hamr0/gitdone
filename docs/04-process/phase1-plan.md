...
| **1.E+** | OTS upgrade cron — periodic `ots upgrade` to anchor pending proofs to Bitcoin | 1.E | Cron every 6h iterates all `ots_proofs/*.ots`, runs `ots upgrade`, commits upgraded proofs back with single git commit per event repo. Idempotent (no change = no commit). Closes the gap where `.ots` files live as calendar-pending proofs that depend on calendar-server longevity until upgraded |
