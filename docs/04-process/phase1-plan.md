...
| **1.H.4** ⊘ | Magic-link management URL + email | 1.H.2 ✅ | **Superseded by 1.H.6** (knowless integration). Originally: opaque 32-hex token, 30-day TTL, file-backed, `/manage/{token}` stub — done 2026-04-19. Replaced 2026-05-02 when the parallel activation-token system collapsed into one knowless Mode A flow. |
| **1.H.5** ✅ | Management dashboard (read-only + close event) | 1.H.4 ✅ | Compact step table with status + deps, attestation/declaration status panels, Send reminders + Close event buttons (303 redirect with flash), email-command fallback footer — **done 2026-04-19**, 2 new integration tests. Routes migrated from `/manage/:token/*` to session-gated `/manage/event/:id/*` in 1.H.6. |
| **1.H.6** ✅ | knowless integration (Mode A activation + Mode B hub) | 1.H.4 ⊘, 1.H.5 ✅ | Replaced gitdone's bespoke activation tokens + manual session minting with one `auth.startLogin` call. Single magic link verifies email, opens 30-day session, lands on dashboard which activates event server-side (mutex-guarded). Net `-733` lines. **done 2026-05-02**, 1 new race regression test. |
