...
Implemented 2026-05-04: tier 1 head tags (description, canonical,
theme-color, OpenGraph quintet, twitter:card) wired through
...
`/manage/callback`, `/manage/verify`, `/events/` (per-event audit
viewer at `/events/:id`) — these are session-gated and don't belong
in search. The `Allow: /events/new` line restores the create form
since longest-match wins for Google.

The privacy-claim invariant (no accounts, no API, no telemetry,
proofs verify offline) is in PRD §0.1 and the landing page footer
chips. The description meta repeats it verbatim so result snippets
self-select the audience. If retention or storage policy changes,
update `DEFAULT_DESCRIPTION` in `templates.js` and the per-route
descriptions in the same commit as the code change.
