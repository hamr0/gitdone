...
git-done.com is **fully retired** (see below). No live events were affected
(completed proofs are archival and verify offline).
...
- **TLS**: one auto-renewing Let's Encrypt cert (webroot) covering
  `signedreply.com`, `mail.signedreply.com`, and `www.signedreply.com` — an
  improvement over git-done.com's manual standalone.
...
- **www.signedreply.com**: A record + cert SAN + nginx 301 → apex (the old
  git-done.com vhost was apex-only, so `www` had never been configured).
- **git-done.com fully retired**: removed from nginx, Postfix, and DNS, and its
  standalone cert deleted. It now returns **NXDOMAIN at the `.com` registry**
  (public delegation lapsed during the migration), so it can't redirect — the
  domain is dead, not merely pointed away. Nothing git-done.com remains on the box.
